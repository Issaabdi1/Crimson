...
class MainPageComment(BasePage):
    comment_input = CommentInput()

    def __init__(self, driver):
        super(MainPageComment, self).__init__(driver)
        self.actions = ActionChains(driver)
        self.thumbs = []
        for i in range(1, 7):
            thumb_locator = getattr(MainPageLocators, f"THUMBNAIL_{i}")
    ...
    # Actions
    def select_text_voice(self):
        element = self.driver.find_element(*MainPageLocators.FIRST_PAGE_THIRD_SPAN)
        time.sleep(2)
        (self.actions.move_to_element(element)
         .move_by_offset(-100, 0)
         .click_and_hold()
    ...
    def select_text_text(self):
        element = self.driver.find_element(*MainPageLocators.FIRST_PAGE_FOURTH_SPAN)
        time.sleep(2)
        (self.actions.move_to_element(element)
         .move_by_offset(-100, 0)
         .click_and_hold()
    ...
    def click_voice_comment_label(self):
        voice_comment_label = self.driver.find_element(*MainPageLocators.SAVED_RECORDINGS_LABEL)
        voice_comment_label.click()